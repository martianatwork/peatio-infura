RSpec.describe Peatio::Infura::Blockchain do
  context :features do
    it 'defaults' do
      blockchain1 = Peatio::Infura::Blockchain.new
      expect(blockchain1.features).to eq Peatio::Infura::Blockchain::DEFAULT_FEATURES
    end

    it 'override defaults' do
      blockchain2 = Peatio::Infura::Blockchain.new(cash_addr_format: true)
      expect(blockchain2.features[:cash_addr_format]).to be_truthy
    end

    it 'custom feautures' do
      blockchain3 = Peatio::Infura::Blockchain.new(custom_feature: :custom)
      expect(blockchain3.features.keys).to contain_exactly(*Peatio::Infura::Blockchain::SUPPORTED_FEATURES)
    end
  end

  context :configure do
    let(:blockchain) { Peatio::Infura::Blockchain.new }
  ...
  context :latest_block_number do
    ...
    let(:blockchain) do
      Peatio::Infura::Blockchain.new.tap {|b| b.configure(server: server)}
    end
  ...
  context :build_transaction do
    ...
    context 'simple infura tx' do
      ...
      let(:blockchain) do
        Peatio::Infura::Blockchain.new.tap { |b| b.configure(currencies: [currency]) }
      end
    ...
    context 'multiple currencies' do
      ...
      let(:blockchain) do
        Peatio::Infura::Blockchain.new.tap do |b|
  ...
  context :fetch_block! do
    ...
    let(:blockchain) do
      Peatio::Infura::Blockchain.new.tap { |b| b.configure(server: server, currencies: [currency]) }
    end
  ...
  context :load_balance_of_address! do
    ...
    context 'undefined currency' do
      it 'raises Peario::Infura::Blockchain::UndefinedCurrencyError' do
        address = 'rHb9CJAWyB4rj91VRWn96DkukG4bwdtyTh'
        expect{ blockchain.load_balance_of_address!(address, :xrp1)}.to raise_error(Peatio::Infura::Blockchain::UndefinedCurrencyError)
      end
    end
