RSpec.describe Peatio::Infura::Client do
  let(:uri) { "http://127.0.0.1:5005" }

  before(:all) { WebMock.disable_net_connect! }
  after(:all) { WebMock.allow_net_connect! }

  subject { Peatio::Infura::Client.new(uri) }
    ...
    context :methodnotfound do
      ...
      it do
        expect{ subject.json_rpc(:methodnotfound) }.to \
          raise_error(Peatio::Infura::Client::ResponseError, "Unknown method. (32)")
      end
    end
    ...
    context :notfound do
      ...
      it do
        expect{ subject.json_rpc(:notfound) }.to \
          raise_error(Peatio::Infura::Client::Error)
      end
    end
    ...
    context :connectionerror do
      ...
      it do
        expect{ subject.json_rpc(:connectionerror) }.to \
          raise_error(Peatio::Infura::Client::ConnectionError)
      end
    end
  end
end
