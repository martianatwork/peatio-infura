...
module Peatio
  module Infura
    class Blockchain < Peatio::Blockchain::Abstract
      ...
      def fetch_block!(block_number)
        ...
        block_json.fetch('transactions').each_with_object([]) do |tx, block_arr|
          if tx.fetch('input').hex <= 0
            ...
            if @erc20.find { |c| c.dig(:options, :erc20_contract_address) == normalize_address(tx.fetch('to')) }.blank?
              next
            end
      ...
      def fetch_tx(tx_id)
        tx = client.json_rpc(:eth_getTransactionByHash, [tx_id])
        return if tx.nil?
        if tx.fetch('input').hex <= 0
          unless invalid_eth_transaction?(tx)
            txs = build_transactions(tx).map do |ntx|
              Peatio::Transaction.new(ntx)
              # ltx.assign_attributes(from_address: normalize_address(tx['from']))
            end
            return txs.first unless txs.nil?
          end
        end
      ...
      def build_erc20_transactions(txn_receipt)
        ...
        txn_receipt.fetch('logs').each_with_object([]) do |log, formatted_txs|
          if log.fetch('topics').blank? || log.fetch('topics')[0] != TOKEN_EVENT_IDENTIFIER
            next
          end
